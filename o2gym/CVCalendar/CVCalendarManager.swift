...
class CVCalendarManager {
    ...
    func weeksWithWeekdaysForMonthDate(date: NSDate) -> (weeksIn: [[Int : [Int]]], weeksOut: [[Int : [Int]]]) {
        ...
        let countOfDaysIn = CVManager.dateRange(lastMonthDateIn).day
        let countOfDaysOut = totalCountOfDays - countOfDaysIn
        
        // Find all dates in.
        var datesIn = [NSDate]()
        for day in 1...countOfDaysIn {
            let components = CVManager.componentsForDate(firstMonthDateIn)
            components.day = day
        ...
        let components = CVManager.componentsForDate(firstWeekDate)
        components.day += 6
        var lastWeekDate = calendar.dateFromComponents(components)!
        
        func nextWeekDateFromDate(date: NSDate) -> NSDate {
            let components = CVManager.componentsForDate(date)
            components.day += 7
        ...
        for i in 0..<count {
            ...
            for weekday in 1...7 {
                ...
                components.day += 1
                let day = CVManager.dateRange(weekdate).day
    ...
    class func dateFromYear(year: Int, month: Int, week: Int, day: Int) -> NSDate? {
        let comps = CVManager.componentsForDate(NSDate())
        comps.year = year
        comps.month = month
